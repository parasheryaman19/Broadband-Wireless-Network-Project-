...
num_bs = sum(1:num_tier)*6; 
 % the positions of the interfering base stations.
freq=channel_bs_ifr3 == 3; % assigning the sub band of interest with a variable 
band = 1:numel(freq) ; % calculating & storing the number of cells allocated with 3rd sub band 
band = band(freq~= 0) ; % excluding the reference cell (0th cell)
post_bs = total_post_bs(1:num_bs); % the positions of the interfering base stations.
post_bs = post_bs(band); % position of interfering base station to cells of sub band 3 in this case
