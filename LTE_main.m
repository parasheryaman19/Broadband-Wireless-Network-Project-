...
% OFDMA Main function of reference LTE simulator.
% Module EE4027, 2014
% Copyright retained
...
for idx_alpha =1: Para.num_alpha
  ...
  for idx_distance=1:Topo.num_distance
    % call function LTE_IFR_sim_dist to calculate the SIR performance of users
    ...
    [Sir_IFR1_distance(idx_alpha, idx_distance), Sir_IFR1_db_distance(idx_alpha, idx_distance)] = ...
      LTE_IFR_sim_dist(temp_distance, Topo.Rc, Topo.num_tier, Topo.num_user_per_ring, alpha, Topo.post_bs,Topo.channel_bs_ifr3);
  ...
  % call OFDMA_IFR_sim_netwide to calculate the SIR performance of users
  % located at fixed distanced away from the reference BS (cell 0).
